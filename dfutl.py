...
def categorize(
    ...
    if len(bins) > 1:
        ...
        if encode:
            ...
            dctCode['toCode'] = \
                {unqBins[cnt]: cnt for cnt in range(0, len(unqBins))}
            dctCode['fromCode'] = \
                {cnt: unqBins[cnt] for cnt in range(0, len(unqBins))}
        ...
        if numCatRet is not None:
            idxAll = np.zeros([len(df.index),], dtype = bool)
            for cnt in range(0, numCatRet):
                if not encode:
                    idx = np.array(df.loc[:,col] >= grouped.index[cnt].left)
                    idx = np.logical_and(idx, np.array(df.loc[:,col] < grouped.index[cnt].right))
                    idxAll = (idxAll) | (idx)
                else:
                    idx = np.array(df.loc[:,col] >= dctCode['fromCode'][grouped.index[cnt]].left)
                    idx = np.logical_and(idx, np.array(df.loc[:,col] < dctCode['fromCode'][grouped.index[cnt]].right))
                    idxAll = (idxAll) | (idx)
